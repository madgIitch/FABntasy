...
describe("notification API authorization",()=>{
 ...
 it("derives the actor from the authenticated session",async()=>{const response=await GET();expect(response.status).toBe(200);expect(mocks.settings).toHaveBeenCalledWith("auth-user",null)});
 it("does not expose settings without a session",async()=>{mocks.getUser.mockResolvedValue({data:{user:null}});const response=await GET();expect(response.status).toBe(401);expect(mocks.settings).not.toHaveBeenCalled()});
