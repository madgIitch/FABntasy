...
describe("notification API authorization",()=>{
 ...
 it("derives the actor from the authenticated session",async()=>{const response=await GET();expect(response.status).toBe(200);expect(mocks.settings).toHaveBeenCalledWith("auth-user")});
