...
export async function GET(request?: Request) { try { const deviceId = request ? new URL(request.url).searchParams.get("deviceId") : null; return response(await notificationSettings(await actor(), deviceId)); } catch (error) { return failure(error); } }
