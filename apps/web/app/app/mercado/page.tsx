...
export default async function MarketPage() {
  ...
  const season=league.competitionSeason; const [market,prices,registrations]=await Promise.all([getMarketContext(actor,league.id),getPlayerPrices({competitionSeasonId:league.competitionSeasonId}),db.playerRegistration.findMany({where:{competitionSeasonId:league.competitionSeasonId},orderBy:{player:{displayName:"asc"}},include:{player:true,teamRegistration:{include:{team:true}},stats:{include:{game:true,fantasyScores:{where:{ruleSet:{status:"ACTIVE"}},orderBy:{createdAt:"desc"},take:1}}}}})]);
