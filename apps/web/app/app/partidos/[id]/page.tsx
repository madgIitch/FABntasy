...
export default async function GamePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!game) notFound();
  return <main className="app-main sports-page"><header className="workspace-header"><div><p className="eyebrow">Jornada {game.roundNumber ?? "–"} · {game.competitionSeason.season.name}</p><h1>Partido</h1></div><span className="live-status"><i /> {game.statsSyncedAt ? `Stats ${date.format(game.statsSyncedAt)}` : `Datos ${date.format(game.lastSeenAt)}`}</span></header>
    <section className="scoreboard"><time>{game.scheduledAt ? date.format(game.scheduledAt) : "Fecha por confirmar"}</time><div><span>{game.homeTeam.name}</span><strong>{game.homeScore ?? "–"}<i>:</i>{game.awayScore ?? "–"}</strong><span>{game.awayTeam.name}</span></div><small>{game.sourceStatus ?? game.status}</small></section>
