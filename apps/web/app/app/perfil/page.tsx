...
export default async function ProfilePage({ searchParams }: { searchParams: Promise<{ account?: string }> }) {
  ...
  return <main className="app-main profile-page">
    ...
    <section className="profile-identity" aria-labelledby="profile-name"><div className="profile-court" aria-hidden="true" /><div className={`profile-avatar${image ? " has-image" : ""}`} style={image ? { backgroundImage: `url(${JSON.stringify(image).slice(1, -1)})` } : undefined}><span aria-hidden="true">{profileInitial(profile.username, profile.displayName)}</span></div><div className="profile-person"><h2 id="profile-name">{profile.username ? `@${profile.username}` : "Completa tu perfil"}</h2><p>{profile.displayName || "Manager de Canastio"}</p></div><div className="profile-stats" aria-label="Resumen fantasy"><div><strong>{profile.leagueCount}</strong><span>{profile.leagueCount === 1 ? "liga" : "ligas"}</span></div><div><strong>{profile.totalPoints === null ? "Sin puntos todavía" : points.format(profile.totalPoints)}</strong><span>{profile.totalPoints === null ? "" : "puntos"}</span></div></div><Link className="profile-edit" href="/app/perfil/editar">Editar perfil <Icon name="chevron" /></Link></section>
    <section className="profile-group" aria-labelledby="leagues-title"><div className="profile-group-heading"><h2 id="leagues-title">Mis ligas</h2><Link href="/app/perfil/ligas">Gestionar</Link></div><div className="profile-list">{profile.leagues.length ? profile.leagues.map(league => <Link className="profile-league-row" key={league.id} href="/app/perfil/ligas"><span className="league-fallback" aria-hidden="true">{league.name.charAt(0).toUpperCase()}</span><span><strong>{league.name}{league.isActive ? " · Activa" : ""}</strong><small>{league.competitionName || "Competición"}</small><small>{league.teamName ?? "Sin equipo todavía"} · {league.memberCount} {league.memberCount === 1 ? "manager" : "managers"}</small></span><Icon name="chevron" /></Link>) : <div className="profile-empty"><p>Todavía no perteneces a ninguna liga.</p><Link href="/app/perfil/ligas">Añadir una liga</Link></div>}</div></section>
