...
type Props = { mode: "login" | "register" | "reset" | "update"; action: (state: AuthState, data: FormData) => Promise<AuthState>; initialMessage?: string; next?: string };
const copy = {
...
export function AuthForm({ mode, action, initialMessage, next = "/app" }: Props) {
  ...
  return <main className="auth-page">
    ...
    <section className="auth-panel">
      ...
      {mode === "login" && <div className="auth-links"><Link href="/recuperar-clave">He olvidado mi contraseña</Link><Link href={`/registro?next=${encodeURIComponent(next)}`}>Crear cuenta</Link></div>}
      {mode === "register" && <div className="auth-links"><span>¿Ya tienes cuenta?</span><Link href={`/login?next=${encodeURIComponent(next)}`}>Iniciar sesión</Link></div>}
