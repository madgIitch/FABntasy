...
async function boot(){let view:ReactNode;
 ...
 case "market-explore":view=<CanastioMarket view="explore" players={players} updatedAt={now} marketV2={{active:true,cycle:{id:"cycle",opensAt:now,closesAt:"2026-09-08T22:00:00Z"},reservedCredits:0,reservedSlots:0,rosterCount:1,rosterSize:8,listings:[{id:"listing-0",playerRegistrationId:players[0].playerRegistrationId,referencePrice:3450000,myBid:null}],history:[]}} market={{leagueId:"demo",leagueName:"Los del viernes",balanceCredits:9000000,clausesOpen:true,clauseCutoffAt:now,serverNow:now}}/>;break;
 case "market-v2":view=<CanastioMarket players={players.map((player,index)=>index===0?{...player,displayName:"RODRÍGUEZ FERNÁNDEZ, PABLO"}:player)} updatedAt={now} marketV2={{active:true,cycle:{id:"cycle",opensAt:now,closesAt:"2026-09-08T22:00:00Z"},reservedCredits:5000000,reservedSlots:1,rosterCount:1,rosterSize:8,listings:players.filter(player=>!player.owner).slice(0,3).map((player,index)=>({id:`listing-${index}`,playerRegistrationId:player.playerRegistrationId,referencePrice:3450000,myBid:index===0?{amountCredits:5000000,status:"ACTIVE"}:null})),history:[{playerRegistrationId:players[3].playerRegistrationId,result:"LOST",winner:"pepe",winningPrice:5300000,bids:[{manager:"pepe",amountCredits:5300000,status:"WON"},{manager:"ana",amountCredits:5000000,status:"LOST"}]}]}} market={{leagueId:"demo",leagueName:"Los del viernes",balanceCredits:9000000,clausesOpen:true,clauseCutoffAt:now,serverNow:now}}/>;break;
