...
async function actor(){const {data:{user}}=await (await createClient()).auth.getUser();if(!user)throw new MarketServiceError("AUTH_REQUIRED",401);return {authUserId:user.id};}
const ok=(data:unknown,status=200)=>NextResponse.json({schemaVersion:FANTASY_MARKET_SCHEMA_VERSION,data,error:null},{status});
const bad=(error:unknown)=>{const known=error instanceof MarketServiceError?error:new MarketServiceError("INVALID_INPUT",422);return NextResponse.json({schemaVersion:FANTASY_MARKET_SCHEMA_VERSION,data:null,error:{code:known.code}},{status:known.status});};
