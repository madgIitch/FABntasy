...
function FormView({ players, metrics }: { players: Player[]; metrics: PlayerMetrics }) {
  const hasHistory = players.some((player) => (metrics[player.playerRegistrationId]?.recentPoints.length ?? 0) > 0);
  if (!hasHistory) return <section className={styles.formEmpty}><p className="eyebrow">Forma reciente</p><h2>Últimos 5 partidos Fantasy</h2><div aria-hidden="true">▁ ▃ ▅ ▆ █</div><strong>Todavía no hay partidos suficientes</strong><p>Los resultados aparecerán aquí a medida que avance la temporada.</p></section>;
  return <section className={styles.formView}><header><div><p className="eyebrow">Forma reciente</p><h2>Últimos 5 partidos Fantasy</h2></div><small>Antiguos → recientes</small></header><ol>{players.map((player) => {
    const history = [...(metrics[player.playerRegistrationId]?.recentPoints ?? [])].reverse();
    const average = history.length ? history.reduce((sum, value) => sum + value, 0) / history.length : null;
    const movement = history.length < 2 ? null : history.at(-1)! - history[0];
    return <li key={player.playerRegistrationId}><div className={styles.formIdentity}><span><strong>{player.displayName}</strong><small>{player.realTeamName}</small></span><b>{average == null ? "—" : average.toFixed(1)}<small>{average == null ? "Sin histórico" : `media · ${formTrend(movement)}`}</small></b></div>{history.length > 0 && <div className={styles.sparkline} aria-label={`Puntos Fantasy, de más antiguo a más reciente: ${history.join(", ")}`}>{history.map((value, index) => <span key={index}><i style={{ "--level": `${Math.max(12, Math.min(100, value * 4))}%` } as CSSProperties} /><small>{value.toFixed(1)}</small></span>)}</div>}</li>;
  })}</ol></section>;
}

function formTrend(movement: number | null) { if (movement == null) return "1 partido"; return movement > 0 ? "↑ Mejorando" : movement < 0 ? "↓ Bajando" : "→ Estable"; }
