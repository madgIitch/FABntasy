...
function PlayerRow({ player, index }: { player: JourneyPlayer; index: number }) { return <details className={styles.player} style={{ "--delay": `${index * 55}ms` } as React.CSSProperties}><summary><b>{String(index + 1).padStart(2, "0")}</b><div><strong>{player.name}</strong><small data-player-state={player.state}><i/>{player.stateLabel}</small></div><span>{player.fantasyPoints === null ? "—" : number.format(Number(player.fantasyPoints))}<small> FP</small></span><em aria-hidden="true">⌄</em></summary><div className={styles.stats}><div><span>PTS</span><strong>{player.points}</strong></div><div><span>AST</span><strong>{player.assists}</strong></div><div><span>ROB</span><strong>{player.steals}</strong></div><p>{player.state === "DNP" ? "No disputó el partido y aporta 0 puntos Fantasy." : player.state === "PENDING" ? "El partido terminó; la puntuación está pendiente de validación." : player.state === "UPCOMING" ? "Su partido todavía no ha comenzado." : "Estadísticas sincronizadas del partido de esta jornada."}</p></div></details>; }
