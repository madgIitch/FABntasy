...
export function LeagueOnboarding({ seasons }: { seasons: Season[] }) {
  ...
  return <main className={`${styles.page} ${styles.onboarding} ${gateStyles.gated}`}>
    ...
    <fieldset className={gateStyles.choice}>
      ...
      <label><input type="radio" name="league-mode" value="create" checked={mode === "create"} onChange={() => choose("create")} /><Icon name="league" /><span><strong>Crear una liga</strong><small>Organiza una competición</small></span></label>
