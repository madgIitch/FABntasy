...
export function LeagueOnboarding({ seasons }: { seasons: Season[] }) {
  ...
  return <main className={`${styles.page} ${styles.onboarding} ${gateStyles.gated}`}>
    ...
    <section className={styles.actions} aria-label="Empezar en Canastio"><div><span className={styles.step}>01</span><h2>Crear una liga</h2><input aria-label="Nombre de la liga" placeholder="Nombre de la liga" value={name} onChange={(event) => setName(event.target.value)} /><select aria-label="Competición" value={seasonId} onChange={(event) => setSeasonId(event.target.value)}>{seasons.map((season) => <option key={season.id} value={season.id}>{season.label}</option>)}</select><input aria-label="Contraseña de la nueva liga" type="password" autoComplete="new-password" minLength={6} maxLength={72} placeholder="Contraseña · mínimo 6 caracteres" value={createPassword} onChange={(event) => setCreatePassword(event.target.value)} /><button disabled={pending || name.trim().length < 3 || !seasonId || createPassword.length < 6} onClick={() => void create()}>Crear liga <span>→</span></button></div>
      <div><span className={styles.step}>02</span><h2>Unirme a una liga</h2><input aria-label="Código de liga" autoCapitalize="characters" placeholder="CNST-XXXXXX" value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} /><input aria-label="Contraseña de liga" type="password" autoComplete="current-password" placeholder="Contraseña" value={joinPassword} onChange={(event) => setJoinPassword(event.target.value)} /><button disabled={pending || !code.trim() || !joinPassword} onClick={() => void join()}>Entrar <span>→</span></button></div></section>
