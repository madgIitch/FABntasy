...
export function ManagerProfile({ leagueId, publicManagerId }: { leagueId: string; publicManagerId: string }) {
  ...
  return <main className={styles.page}>
    ...
    <nav className={styles.actions} aria-label="Acciones del perfil"><a href={`/api/leagues/${leagueId}/head-to-head?manager=${encodeURIComponent(publicManagerId)}`}>Head-to-Head</a><button disabled={!data.rivalryAvailable}>Rivalidad</button><button onClick={() => void share()}>Compartir perfil</button></nav>
