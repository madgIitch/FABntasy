...
describe("active league selection contract", () => {
  it("validates an ACTIVE membership and falls back deterministically", () => {
    ...
    expect(service).toContain('status: "ACTIVE", league: { status: "ACTIVE", legacyTeamId: null, OR: [{ competitionSeason: { fantasyEnabled: true } }, { selectedSeasons: { some: { competitionSeason: { fantasyEnabled: true } } } }] }');
