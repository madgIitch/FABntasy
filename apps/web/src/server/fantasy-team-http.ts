...
export const fantasyOk = (data: unknown, status = 200) => NextResponse.json({ schemaVersion: FANTASY_TEAM_SCHEMA_VERSION, data }, { status });
...
export function fantasyError(error: unknown) {
  ...
  return NextResponse.json({ schemaVersion: FANTASY_TEAM_SCHEMA_VERSION, error: { code: known.code } }, { status: known.status });
}
