...
export async function getJourney(authUserId:string,requestedRound?:number):Promise<JourneyData|null>{
 ...
 const [lineup,roundScore,correction]=await Promise.all([
  ...
  db.sportsDataRevision.findFirst({where:{status:"APPLIED",publicReason:{not:null},OR:[{game:{competitionSeasonId:team.competitionSeasonId,roundNumber}},{playerGameStat:{game:{competitionSeasonId:team.competitionSeasonId,roundNumber}}}]},orderBy:{appliedAt:"desc"},select:{appliedAt:true,publicReason:true}}),
 ]);
