...
export async function resolveActiveLeagueId(actor: LeagueActor) {
  ...
  const memberships = await db.leagueMembership.findMany({ where: { userProfileId: profile.id, status: "ACTIVE", league: { status: "ACTIVE", legacyTeamId: null, competitionSeason: { fantasyEnabled: true } } }, orderBy: [{ joinedAt: "desc" }, { id: "asc" }], select: { leagueId: true } });
...
export async function selectActiveLeague(actor: LeagueActor, leagueId: string) {
  ...
  if (!profile) fail("LEAGUE_NOT_FOUND", 404);
  const membership = await db.leagueMembership.findFirst({ where: { userProfileId: profile.id, leagueId, status: "ACTIVE", league: { status: "ACTIVE", legacyTeamId: null, competitionSeason: { fantasyEnabled: true } } }, select: { id: true } });
