...
export async function listActivity(actor:SocialActor,leagueId:string,input:{cursor?:string|null;limit?:number}={}){
  ...
  return {items:page.map(row=>{const counts=Object.fromEntries(REACTION_CATALOG.map(emoji=>[emoji,row.reactions.filter(r=>r.emoji===emoji).length]));return {id:row.id,type:row.type,occurredAt:row.occurredAt.toISOString(),actor:row.actorProfile?{name:row.actorProfile.username?`@${row.actorProfile.username}`:row.actorProfile.displayName??"Manager",publicManagerId:row.actorProfile.username}:null,payload:row.payload,reactions:counts,myReactions:row.reactions.filter(r=>r.userProfileId===profileId).map(r=>r.emoji)};}),nextCursor:hasMore?encodeActivityCursor(page[page.length-1].occurredAt,page[page.length-1].id):null,reactionCatalog:REACTION_CATALOG};
}
...
export async function getMembers(actor:SocialActor,leagueId:string,input:{cursor?:string|null;limit?:number}={}){
  ...
  const items=await Promise.all(page.map(async member=>{const team=teams.find(t=>t.userProfileId===member.userProfileId);const wins=team?.roundScores.filter(score=>{const max=Math.max(...teams.map(t=>Number(t.roundScores.find(r=>r.roundNumber===score.roundNumber)?.points??Number.NEGATIVE_INFINITY)));return Number(score.points)===max;}).length??0;return {membershipId:member.id,publicManagerId:member.userProfile.username,name:member.userProfile.username?`@${member.userProfile.username}`:member.userProfile.displayName??"Manager",role:member.role,teamName:team?.name??"Equipo pendiente",position:team?ordered.findIndex(t=>t.id===team.id)+1:null,currentRosterValue:team?team.rosterSlots.reduce((sum,s)=>sum+Number(s.playerRegistration.prices[0]?.currentPrice??0),0):null,totalPoints:team?.total?Number(team.total.totalPoints):null,roundsWon:wins,bestRound:team?.roundScores.length?Math.max(...team.roundScores.map(s=>Number(s.points??0))):null,streak:null,trophies:team?awaitAwards(team.userProfileId,leagueId):[]};}));return {items,nextCursor:members.length>limit?page[page.length-1].id:null};
}
