...
export class SocialLeagueError extends Error { constructor(public code: "AUTH_REQUIRED"|"LEAGUE_NOT_FOUND"|"EVENT_NOT_FOUND"|"MEMBER_NOT_FOUND"|"PLAYER_NOT_FOUND"|"INVALID_INPUT"|"FEATURE_DISABLED", public status=400){super(code);} }
...
export async function getMembers(actor:SocialActor,leagueId:string,input:{cursor?:string|null;limit?:number}={}){
  ...
  const items=await Promise.all(page.map(async member=>{const team=teams.find(t=>t.userProfileId===member.userProfileId);const wins=team?.roundScores.filter(score=>{const max=Math.max(...teams.map(t=>Number(t.roundScores.find(r=>r.roundNumber===score.roundNumber)?.points??Number.NEGATIVE_INFINITY)));return Number(score.points)===max;}).length??0;return {membershipId:member.id,userProfileId:member.userProfileId,name:member.userProfile.username?`@${member.userProfile.username}`:member.userProfile.displayName??"Manager",avatarPath:member.userProfile.avatarPath,role:member.role,teamName:team?.name??"Equipo pendiente",position:team?ordered.findIndex(t=>t.id===team.id)+1:null,currentRosterValue:team?team.rosterSlots.reduce((sum,s)=>sum+Number(s.playerRegistration.prices[0]?.currentPrice??0),0):null,totalPoints:team?.total?Number(team.total.totalPoints):null,roundsWon:wins,bestRound:team?.roundScores.length?Math.max(...team.roundScores.map(s=>Number(s.points??0))):null,streak:null,trophies:team?awaitAwards(team.userProfileId,leagueId):[]};}));return {items,nextCursor:members.length>limit?page[page.length-1].id:null};
}
