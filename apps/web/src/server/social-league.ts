...
async function memberContext(client:Prisma.TransactionClient|typeof db,actor:SocialActor,leagueId:string){
  ...
  const membership=await client.leagueMembership.findFirst({where:{leagueId,userProfileId:profile.id,status:"ACTIVE",league:{status:"ACTIVE",competitionSeason:{fantasyEnabled:true}}},select:{id:true}});
...
export async function togglePlayerFollow(actor:SocialActor,playerRegistrationId:string,follow:boolean){const profile=await db.userProfile.findUnique({where:{authUserId:actor.authUserId},select:{id:true}});if(!profile)throw new SocialLeagueError("AUTH_REQUIRED",401);const player=await db.playerRegistration.findFirst({where:{id:playerRegistrationId,competitionSeason:{fantasyEnabled:true}},select:{competitionSeasonId:true}});if(!player)throw new SocialLeagueError("PLAYER_NOT_FOUND",404);return db.$transaction(async tx=>{await requireFantasyCompetition(tx,player.competitionSeasonId);if(follow)await tx.playerFollow.upsert({where:{userProfileId_playerRegistrationId:{userProfileId:profile.id,playerRegistrationId}},create:{userProfileId:profile.id,playerRegistrationId,competitionSeasonId:player.competitionSeasonId},update:{}});else await tx.playerFollow.deleteMany({where:{userProfileId:profile.id,playerRegistrationId}});return {following:follow};});}
