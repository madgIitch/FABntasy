...
describe("sports data revision persistence",()=>{
 ...
 it("records both successful and failed recomputation retries",()=>{const service=readFileSync(resolve(root,"apps/web/src/server/sports-data-revisions.ts"),"utf8");expect(service).toContain('action:"DATA_REVISION_RETRY"');expect(service).toContain('result:"SUCCESS"');expect(service).toContain('result:"FAILED"')});
});
