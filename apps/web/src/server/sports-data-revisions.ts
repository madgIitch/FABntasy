...
export async function retryRevisionRecomputation(actorProfileId:string,id:string){if(!uuid(id))throw new RevisionError("INVALID_REVISION",422);const revision=await db.sportsDataRevision.findUnique({where:{id}});if(!revision||revision.status!=="APPLIED")throw new RevisionError("REVISION_NOT_FOUND",404);if(revision.recomputationStatus!=="FAILED")throw new RevisionError("INVALID_STATE",409);const impact=revision.impact as {competitionSeasonId:string;roundNumber:number|null};await db.sportsDataRevision.update({where:{id},data:{recomputationStatus:"RUNNING",recomputationError:null}});try{if(impact.roundNumber){const rule=await db.fantasyScoringRuleSet.findFirst({where:{competitionSeasonId:impact.competitionSeasonId,status:"ACTIVE"},orderBy:{publishedAt:"desc"}});if(rule){await recomputeRound(impact.competitionSeasonId,impact.roundNumber,rule.id);await recomputePlayerPrices(impact.competitionSeasonId,impact.roundNumber);}}await db.$transaction([db.sportsDataRevision.update({where:{id},data:{recomputationStatus:"SUCCEEDED"}}),db.adminAuditEvent.create({data:{actorProfileId,action:"DATA_REVISION_RETRY",resourceType:"SPORTS_DATA_REVISION",resourceId:id,result:"SUCCESS"}})]);return {id,recomputationStatus:"SUCCEEDED"};}catch{await db.$transaction([db.sportsDataRevision.update({where:{id},data:{recomputationStatus:"FAILED",recomputationError:"RECOMPUTATION_FAILED"}}),db.adminAuditEvent.create({data:{actorProfileId,action:"DATA_REVISION_RETRY",resourceType:"SPORTS_DATA_REVISION",resourceId:id,result:"FAILED"}})]);throw new RevisionError("RECOMPUTATION_FAILED",503);}}
