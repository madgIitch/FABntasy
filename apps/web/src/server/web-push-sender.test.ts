...
describe("VAPID production configuration",()=>{
 ...
 it("rejects malformed subjects and versions",()=>{for(const subject of ["ops@example.test","mailto:","https://","https://user:password@example.test"]){expect(()=>validateVapidConfiguration({...valid,VAPID_SUBJECT:subject})).toThrow("VAPID_SUBJECT_INVALID")}expect(()=>validateVapidConfiguration({...valid,VAPID_KEY_VERSION:"spaces forbidden"})).toThrow("VAPID_KEY_VERSION_INVALID")});
});
