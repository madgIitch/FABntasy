...
BEGIN
  ...
  IF v_scenario='account.multi-league' AND (SELECT count(*) FROM fantasy_teams WHERE user_profile_id=md5(v_run || ':profile:1')::uuid)<>2 THEN RAISE EXCEPTION '14F multi-league isolation fixture missing'; END IF;
END $$;
